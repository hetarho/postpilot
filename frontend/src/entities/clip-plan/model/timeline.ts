...
import { CLIP_ACCENTS } from '@/entities/clip-template/@x/clip-plan'
import {
  copyClipPlan,
  editClipPlan,
  validateClipPlan,
...
/** What the evidence check needs of an observation record, stated by the reader rather than
 *  imported from the observation noun: the plan is below observations in the `@x` graph, and a
 *  `ClipObservations` satisfies this shape. */
export interface ClipCutEvidence {
  status: string
  sources: readonly {
    source: { id: string; fingerprint: string }
    segments: readonly { usability: string; startMs: number; endMs: number }[]
  }[]
}
