...
clip children: field, group, guide, stage, text. A template declares only what every clip made from it must carry: the design, the intro/outro slot text, an optional badge, the information to collect, an optional ordered list of named composition stages and one optional guide. Footage order, cut rhythm and every caption are decided at generation from the project instruction and the observed footage; caption pace and accent are chosen per project.
...
stage(name) = one line of intent. An ordered list of named composition stages the flow follows where the footage allows: it admits and forbids no footage, a stage nothing was filmed for is skipped or merged, and footage matching no stage is kept where it belongs. Root children only, never inside a scene or repetition. Name up to the label limit, intent up to the prompt limit.
