...
export function toStructured(p: ProtoStructured | undefined): StructuredVoiceProfile {
  return {
    ...
    axes: { involvement: p?.axes?.involvement ?? 0, narrativity: p?.axes?.narrativity ?? 0, persuasionOvertness: p?.axes?.persuasionOvertness ?? 0, abstractness: p?.axes?.abstractness ?? 0, addresseeFocus: p?.axes?.addresseeFocus ?? 0, humor: p?.axes?.humor ?? 0 },
