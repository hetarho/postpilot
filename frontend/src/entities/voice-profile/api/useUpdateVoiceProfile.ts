...
export function useUpdateVoiceProfile(ownerId: string) {
  const transport = useTransport()
  const queryClient = useQueryClient()
  const mutationKey = [...VOICE_PROFILE_UPDATE_KEY, ownerId] as const
  ...
  return {
    ...
    saveStyleguide: (styleguide: string) => mutation.mutateAsync({ styleguide }),
    saveRules: (rules: string) => mutation.mutateAsync({ rules }),
  }
}
