  ...
  return (
    <section aria-label={t('preview.title')} className={compact ? 'contents' : 'space-y-3'}>
      ...
      <Typography
        variant="body"
        role="status"
        className={compact && !preview.failure ? 'sr-only' : 'text-content-secondary'}
      >
        {preview.failure
          ? t('preview.preparationFailed')
          : !preview.ready || preview.updating
            ? t('preview.updating')
            : t('preview.currentDraft')}
      </Typography>
      {preview.failure && preview.failure.reason !== 'UNKNOWN_FAILURE' && (
        <AppFailureMessage failure={preview.failure} />
      )}
      {preview.failure && (
        <Button variant="secondary" onClick={preview.onRetry}>
          {t('preview.retry')}
        </Button>
      )}
