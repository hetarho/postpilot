export const clips = {
  ...
  composition: {
    ...
    stageHint: '찍힌 영상이 허락하는 만큼만 따라가요. 해당하는 영상이 없으면 그 단계는 건너뛰어요.',
    kindLabel: '문구 작성 방식',
    roleLabel: '문구 용도',
    positionLabel: '화면 위치',
    alignLabel: '글자 정렬',
    ...
    errors: {
      ...
      unsupported_role:
        '자막(caption)과 정보(info) 문구는 더 이상 템플릿에 쓰지 않아요. 자막은 클립을 만들 때 지침과 영상으로 정해요.',
      unsupported_basis:
        '컷 기준 시간(basis="cut")은 더 이상 쓸 수 없어요. whole이나 출력 시작·끝 기준을 사용해 주세요.',
      ...
      invalid_max: '최대 글자 수는 1 이상이어야 하고, 그 자리가 허용하는 글자 수보다 클 수 없어요.',
      answer_limit_field:
        '{{label}}은(는) {{max}}자까지예요. 지금 {{actual}}자라서 그대로 만들 수 없어요.',
    },
    guide:
      '아래 문법으로 postpilot 영상 템플릿 원문만 작성하세요. 외부 AI에서 만든 원문을 붙여넣어 사용할 수 있습니다. 원문 작성·복사·해석·예시 미리보기에는 모델 호출이나 크레딧 사용이 없습니다. 루트의 intro·caption·outro를 먼저 선택하세요. 인트로·아웃트로의 슬롯 순서와 모양은 고정이며 슬롯 문구, 배지, 정보 필드, 항목 그룹, 안내문만 작성할 수 있습니다. 템플릿은 모든 클립에 반드시 들어갈 것만 선언합니다. 장면 순서, 컷 리듬, 자막의 내용과 개수는 클립을 만들 때 프로젝트 지침과 실제 영상이 정하고, 자막 속도와 강조색은 프로젝트마다 고릅니다. 필수 가격이나 자동 광고 문구, CTA는 없습니다.\n\n문법:\n{{grammar}}\n\n상한(문자 수는 유니코드 글자 수, Ms는 밀리초):\n{{limits}}\n\nfield의 label은 화면 이름, 본문은 입력 안내입니다. ID로 연결하므로 이름 변경이 항목의 정체성을 바꾸지 않습니다. 선택 정보가 비어 있으면 그 정보에 연결된 문구가 생략됩니다. 인트로·아웃트로에서는 해당 슬롯만 비우고 다른 슬롯의 위치를 유지합니다. 필수 정보가 비면 생성을 시작할 수 없습니다. 각 항목의 가격·통화·단위·가격 기준은 같은 항목 안에 적고 다른 항목에 섞지 마세요. 항목의 이름과 가격은 화면 라벨이 아니라 내레이션이 말하는 사실이 되고, 숫자는 입력된 값과 정확히 일치할 때만 자막에 들어갑니다. fixed 문구는 value 치환을 제외하면 공백까지 그대로 보존합니다. guide는 출력에 보이지 않으며 말투와 시점처럼 매번 반복하기 귀찮은 안내를 담습니다. 프로젝트 지침이 먼저이고 guide는 그 뒤에 붙습니다.\n\n예시(입력 화면에서 메뉴 항목을 2개 이상 추가할 수 있음):\n{{example}}',
  },
  credits: {
    writingCalls: '작성 호출',
    call: {
  ...
  project: {
    ...
    captionStylesHelp: '고른 스타일 중에서 자막마다 하나씩 쓰여요. ②에서 자막마다 바꿀 수 있어요.',
