  ...
  return (
    <main className="mx-auto w-full max-w-2xl px-6 py-6">
      <div className="flex items-center justify-between">
        ...
        <Link
          to="/posts/new"
          className="rounded-md bg-neutral-100 px-3 py-1.5 text-xs font-medium text-neutral-900"
        >
          새 글
        </Link>
      </div>
      ...
      {!isError && isPending && <p className="mt-8 text-sm text-neutral-500">불러오는 중…</p>}

      {!isError && !isPending && posts.length === 0 && (
        <p className="mt-8 text-sm text-neutral-500">아직 글이 없어요. "새 글"로 시작해 보세요.</p>
      )}
        ...
        {posts.map((post) => (
          <li key={post.slug}>
            <Link
              ...
              className="flex items-center gap-3 py-3 hover:bg-neutral-900/40"
            >
              ...
              <time dateTime={post.updatedAt} className="shrink-0 text-xs text-neutral-500">
