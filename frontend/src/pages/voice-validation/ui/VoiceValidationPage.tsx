    ...
    <main className="mx-auto w-full max-w-4xl px-4 py-6 sm:px-6">
      ...
      <div className="mt-4 flex flex-wrap items-baseline justify-between gap-2">
        <h1 className="text-2xl font-semibold">
          프로필 검증 v{validation.profileVersion.toString()}
        </h1>
        <Badge
          tone={
      ...
      {canRetry && (
        <Button
          ...
          onClick={() => void retry.mutateAsync({ validationId: id }).then(() => query.refetch())}
        >
          실패한 항목 다시 실행
        </Button>
      )}
      {validation.judgeEnabled && validation.totalCount > 0 && (
        <Notice tone="info" className="mt-4">
          5개 항목 일치율: {Math.round((validation.yCount / validation.totalCount) * 100)}% (
          {validation.yCount}/{validation.totalCount})
        </Notice>
      )}
      <div className="mt-6 space-y-8">
        {validation.items.map((item, index) => (
          <article key={item.id}>
            <h2 className="font-semibold">검증 글 {index + 1}</h2>
            {item.error && (
            ...
            <div className="mt-3 grid gap-4 md:grid-cols-2">
              <section className="bg-surface-raised rounded-lg p-4">
                <h3 className="text-sm font-medium">원문</h3>
                <p className="mt-2 text-sm leading-relaxed whitespace-pre-wrap">{item.original}</p>
              </section>
              <section className="bg-surface-raised rounded-lg p-4">
                <h3 className="text-sm font-medium">중립 주제 요약</h3>
                <p className="text-content-secondary mt-2 text-sm">
                  {item.neutralSummary || '생성 중…'}
                </p>
                <h3 className="mt-4 text-sm font-medium">프로필로 다시 쓴 글</h3>
                <p className="mt-2 text-sm leading-relaxed whitespace-pre-wrap">
                  {item.regenerated || '생성 중…'}
                </p>
