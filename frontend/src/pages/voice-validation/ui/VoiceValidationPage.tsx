...
import { voiceValidationQueryKey } from '@/entities/voice-profile'
...
export function VoiceValidationPage() {
  const { id } = useParams({ from: '/authenticated/voice/validations/$id' })
  const { user } = useSession()
  const transport = useTransport()
  const key = voiceValidationQueryKey(transport, user?.id ?? '', id)
  const query = useQuery({ queryKey: key, queryFn: () => createClient(VoiceValidationService, transport).getVoiceProfileValidation({ validationId: id }), refetchInterval: (state) => ['queued', 'running'].includes(state.state.data?.validation?.status ?? '') ? POLL_INTERVAL_MS : false })
