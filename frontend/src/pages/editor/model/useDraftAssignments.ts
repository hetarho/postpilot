...
import type { BlogFieldChoice } from '@/entities/blog-field'
...
/** What a draft is written WITH: its voice, its template, its 분야 and the language it is written
 *  in.
 *
 *  For a saved post the server owns all four, and each changes only through a confirmed
 *  assignment (`useAutosave.reassign`). For `/posts/new` there is no post to own them, so the
 *  screen holds the choices until the first save mints one. The language is snapshotted once
 *  when the draft opens: later interface-locale switches are presentation-only, and the explicit
 *  selector is the sole way this draft's target changes. */
export function useDraftAssignments(post: PostDraft | undefined, defaultVoiceId: string) {
  ...
  const [field, setField] = useState<BlogFieldChoice>('')
