...
function scanLine(line, { isTokenFile = false } = {}) {
  const findings = []
...
function runProbe() {
  ...
  console.log(`lint:style:probe — ok (${cases.length} scanner cases)`)
}
...
for (const file of walk(ROOT)) {
  ...
  lines.forEach((line, index) => {
    for (const { kind, match } of scanLine(line, { isTokenFile })) {
