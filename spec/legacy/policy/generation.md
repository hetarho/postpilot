...
- A `Block` carries an optional typed `slot {kind, label}`, set **only** on an unfilled template slot. It is a field
  on `TEXT` rather than a sixth `BlockType`: a new enum member would force every existing switch — reading view,
  block editor, the four export mappings, revision, this validator — to change merely to stay correct, while an
  optional field means only the surfaces that render a slot specially need to know it exists. [I2] holds either way.
- One template pass runs **after** validation and the attachment filter, and only when a template was frozen: it
  resolves the copy tokens the model was asked to reproduce into slot blocks, and **inserts any slot the model
  omitted at its template position** — after the nearest preceding slot that resolved, otherwise before the nearest
  following one, otherwise at the end. A block holding several tokens resolves every slot it named and keeps the
  prose between them; a repeated token resolves once. The pass is idempotent, and it runs again after a revision.
  It verifies nothing else: literal fidelity and section order are not checked, and no template deviation drops a
  block, fails a run or triggers a retry.
...
- Ordinary generation is a durable `generate` job. `StartGeneration` validates ownership and one explicit active
  ...
  optional target length, the post's optional template brief, **and the account's applicable writing-guideline texts**
...
- **The selection is frozen at enqueue, with presence.** The start carries a `ReobserveSelection` message whose
  ...
  length, template and guidelines. Editing photos or switching models afterwards cannot change what a queued run
...
- The writing prompt's stable prefix is static task/format rules → target-specific baseline when applicable → voice
  projection → optional frozen template brief (see [templates](templates.md)) → optional frozen `[작문 지침]` section
...
- The template brief is resolved from the post once, at enqueue, through the template context's published lookup, and
  written into the payload as text. Handlers build the prompt from that payload and never re-read the row, so editing
  or deleting the template afterwards — including across a restart-resume or an explicit retry — cannot change work
  already queued. A template deleted before the start is simply absent. The observe stage never receives a brief.
- The applicable guideline texts are resolved at the same moment, from the same template id, through the guideline
  context's published `ForPrompt`, and frozen the same way. When non-empty they render as exactly one `[작문 지침]`
  section — after the template section when the post has one, otherwise directly after `[종결어미 제약]`, always before
  the per-post material — closed by a fixed precedence sentence that makes a guideline outrank a conflicting template
...
- The A/B snapshot freezes the same target, voice projection, brief, and guideline texts, so both candidates get
  byte-identical inputs except for model ref. The input hash changes with target, template, or the applicable guideline
  set, and the experiment records both target and `template_name`; the guideline texts are not projected for display.
...
- Everything the next run is GIVEN is set in 글 생성's dock and rendered nowhere else in the editor
  ...
  single options trigger, and 말투 and 템플릿 on the dock's own row beside it. The A/B pair is set THERE rather than
