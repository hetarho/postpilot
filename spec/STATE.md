...
| T115 | Cut length becomes a target, not a refusal | CDS CLIP | - | todo |
| T116 | The caption repair ladder | CDS CLIP | - | todo |
...
- `release-smoke` is red on HEAD independently of T111/T112 (verified 260912 on a worktree of the committed HEAD): `source 0 downloads=4 want=2` in every rendering mode and `persisted plan still has model timing errors` in multi-source-timing — T115 restores the timing fixture; the double source download needs a fix task or T116
- implement-task T115 next, then T116 — both take the clip chain OFF the failure path: T115 makes CDS-37 a target the compiler aims at, T116 makes a failing verifier check cost a caption instead of the clip. Neither needs a new port or a contract change. **r2 never reached code**, so nothing has to be un-implemented
