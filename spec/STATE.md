...
- the clip chain has no failure path left for a caption or a cut length (T115 T116); what remains open is T110 (owner-blocked) and T008 (owner-present). Run `release-smoke` (see the T115 result) before `done` on anything touching the compiler, verifier, renderer or admission. Two CDS readings still want an update-ssot: footage-bound delivery below the target (floor 15 s) and CDS-40's run rule read against the approved set
