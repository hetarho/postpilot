...
| VIDEO | 2 | 2 | - | 1 |
| CLIP | 6 | 6 | - | 0 |
| CDS | 2 | 1 | CDS-36✎ CDS-37✎ CDS-50✎ | 2 |
| BILL | 4 | 4 | - | 0 |
...
- create-task CDS next: r2 moves the cut range and the fade ratio to the category preset, and T108 hard-codes both (`design.CutBounds` reads only the global numbers, `design.Transitions` only 0.4) — the compiler has the preset on `EditPlan.Preset` already, so this is a rule change, not a new port
