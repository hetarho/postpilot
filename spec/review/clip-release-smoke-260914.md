...
> st:ready@260914 | scope:backend/cmd/api (release smoke), backend/internal/clip/ai/schemas | at:0c6a07f | base:ARCH@2
...
- F2 [?] P2 `backend/cmd/api/clip_release_test.go:143` + `Dockerfile:65`: the release smoke runs only when `CLIP_RELEASE_SMOKE=1`, which is set only inside the image build, so ARCH-26's `go test ./...` skips the one check that gates deployment ← T145 passed every local gate and still broke the pipeline, and a developer cannot reproduce it: the harness wants the bundled faces at their absolute runtime paths and `prepare` wants ffmpeg. Whether local verify should be able to run it is an ARCH decision (ARCH-26, ARCH-31), so this is left open rather than tasked
