...
func TestClipAccountingDistinguishesPendingZeroLegacyAndMaster(t *testing.T) {
	for _, tier := range []plan.Plan{plan.Free, plan.Master} {
		t.Run(string(tier), func(t *testing.T) {
			...
			if a, err := svc.ReservationAccounting(ctx, "alice", "clip"); err != nil || a != nil {
				t.Fatal(a, err)
			}
			start := holdFor("clip")
			start.Kind = "generate_clip"
			start.Plan = tier
			start.Approval = approvedStoreClip()
			...
			if a, err := svc.ReservationAccounting(ctx, "bob", "clip"); err != nil || a != nil {
	...
	a, err := svc.ReservationAccounting(context.Background(), "alice", "legacy")
...
func TestSQLiteCeilingRefusalAndOverageStayWithinApproval(t *testing.T) {
	...
	start.Kind = "generate_clip"
	start.Approval = approvedStoreClip()
	start.Approval.ApprovedMaxCredits = 4
