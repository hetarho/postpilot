...
func mustCreatePost(t *testing.T, svc *Service, userID, title string) Post {
	...
	created, err := svc.SaveDraft(context.Background(), userID, DraftSave{Title: title, VoiceID: &voiceID, TargetLanguage: &language})
	if err != nil {
...
func TestSaveDraftCreatesThenUpdates(t *testing.T) {
	...
	voiceID := aliceVoice
	language := LanguageKorean
	created, err := svc.SaveDraft(ctx, alice, DraftSave{Title: "Jeju", Memo: "first", VoiceID: &voiceID, TargetLanguage: &language})
	if err != nil {
	...
	updated, err := svc.SaveDraft(ctx, alice, DraftSave{Slug: created.Slug, Title: "Jeju", Memo: "second"})
	if err != nil {
...
func TestSaveDraftKeepsTheSlugOnRetitle(t *testing.T) {
	...
	renamed, err := svc.SaveDraft(context.Background(), alice, DraftSave{Slug: created.Slug, Title: "Something else entirely"})
	if err != nil {
		t.Fatalf("SaveDraft: %v", err)
	}
...
func TestOwnership(t *testing.T) {
	...
	t.Run("foreign post is forbidden, not hidden", func(t *testing.T) {
		...
		if _, err := svc.SaveDraft(ctx, bob, DraftSave{Slug: mine.Slug, Title: "x", Memo: "y"}); !errors.Is(err, ErrForbidden) {
	...
	t.Run("unknown slug is not found", func(t *testing.T) {
		...
		if _, err := svc.SaveDraft(ctx, alice, DraftSave{Slug: "nope", Title: "x", Memo: "y"}); !errors.Is(err, ErrNotFound) {
...
func TestWinnerApplicationIsIdempotentAtPostBoundary(t *testing.T) {
	...
	if _, err := svc.SaveContent(context.Background(), alice, found.Slug, manual, first.ContentRevision, nil); err != nil {
		t.Fatal(err)
	}
...
func TestCreatePostRetriesWhenTheSlugIsTakenMidFlight(t *testing.T) {
	...
	voiceID := aliceVoice
	language := LanguageKorean
	created, err := svc.SaveDraft(ctx, alice, DraftSave{Title: "Jeju", VoiceID: &voiceID, TargetLanguage: &language})
	if err != nil {
		t.Fatalf("SaveDraft: %v", err)
	}
