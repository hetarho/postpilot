...
func TestSaveDraftAssignsClearsOrPreservesTheTemplate(t *testing.T) {
	...
	kept, err := svc.SaveDraft(ctx, alice, DraftSave{Slug: created.Slug, Title: "Jeju", Memo: "memo 2"})
	...
	cleared, err := svc.SaveDraft(ctx, alice, DraftSave{Slug: created.Slug, Title: "Jeju", Memo: "memo 3", TemplateID: &blank})
	...
	reassigned, err := svc.SaveDraft(ctx, alice, DraftSave{Slug: created.Slug, Title: "Jeju", Memo: "memo 4", TemplateID: &review})
...
func TestSaveDraftRejectsAnUnknownOrForeignTemplateAndAppliesNothingElse(t *testing.T) {
	...
	for name, id := range map[string]string{"unknown": "template-nobody", "foreign": "template-bob"} {
		t.Run(name, func(t *testing.T) {
			bad := id
			if _, err := svc.SaveDraft(ctx, alice, DraftSave{Slug: created.Slug, Title: "새 제목", Memo: "새 메모", TemplateID: &bad}); !errors.Is(err, ErrTemplateNotFound) {
			...
			language := LanguageKorean
			if _, err := svc.SaveDraft(ctx, alice, DraftSave{Title: "새 글", VoiceID: &voiceID, TemplateID: &bad, TargetLanguage: &language}); !errors.Is(err, ErrTemplateNotFound) {
...
func TestAssigningATemplateTouchesNoContentOrFinalizationState(t *testing.T) {
	...
	review := "template-review"
	after, err := svc.SaveDraft(ctx, alice, DraftSave{Slug: created.Slug, Title: before.Title, Memo: before.Memo, TemplateID: &review})
	if err != nil {
...
func TestGetAndListProjectTheTemplateName(t *testing.T) {
	...
	voiceID := defaultVoiceFor(alice)
	language := LanguageKorean
	created, err := svc.SaveDraft(ctx, alice, DraftSave{Title: "Jeju", VoiceID: &voiceID, TemplateID: &review, TargetLanguage: &language})
	if err != nil {
		t.Fatal(err)
	}
...
func TestWithoutATemplateDirectoryPostsStillWorkAndAssignmentIsRefused(t *testing.T) {
	...
	review := "template-review"
	if _, err := svc.SaveDraft(ctx, alice, DraftSave{Slug: created.Slug, Title: "Jeju", TemplateID: &review}); err == nil {
