...
func TestCreateRequiresAnOwnedActiveVoice(t *testing.T) {
	...
	for name, tc := range map[string]struct {
		...
		t.Run(name, func(t *testing.T) {
			if _, err := svc.SaveDraft(ctx, alice, "", "Jeju", "", tc.voice); !errors.Is(err, tc.want) {
	...
	voiceID := aliceVoice
	if _, err := bare.SaveDraft(ctx, alice, "", "Jeju", "", &voiceID); err == nil {
...
func TestReassignmentPreservesContentAndClearsTheBaselineVoice(t *testing.T) {
	...
	review := aliceReview
	moved, err := svc.SaveDraft(ctx, alice, created.Slug, "Jeju", "memo", &review)
	if err != nil {
	...
	same := aliceReview
	if _, err := svc.SaveDraft(ctx, alice, created.Slug, "Jeju", "memo 2", &same); err != nil {
...
func TestReassignedReviewCanFinalizeWithoutPublishingLearningEvidence(t *testing.T) {
	...
	target := aliceReview
	moved, err := svc.SaveDraft(ctx, alice, created.Slug, created.Title, created.Memo, &target)
...
func TestReassignmentTargetsAndBusyPostsAreRefused(t *testing.T) {
	...
	for name, tc := range map[string]struct {
		...
		t.Run(name, func(t *testing.T) {
			voiceID := tc.voice
			if _, err := svc.SaveDraft(ctx, alice, created.Slug, "Jeju", "", &voiceID); !errors.Is(err, tc.want) {
	...
	if _, err := svc.SaveDraft(ctx, alice, created.Slug, "Jeju", "", &review); !errors.Is(err, ErrPostBusy) {
	...
	if moved, err := svc.SaveDraft(ctx, alice, created.Slug, "Jeju", "", &review); err != nil || moved.VoiceID != aliceReview {
		t.Fatalf("idle reassign = %+v err=%v", moved, err)
	}
	// A title-only autosave arriving afterwards preserves the newer assignment.
	if kept, err := svc.SaveDraft(ctx, alice, created.Slug, "Jeju 2", "", nil); err != nil || kept.VoiceID != aliceReview {
