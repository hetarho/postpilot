...
func TestSimpleOffersTopWhenBottomCoversSubject(t *testing.T) {
	rule := design.Styles["simple"]
	if rule.Anchor != "bottom" || rule.AnchorAlt != "top" {
...
func TestObservedSpaceRanksOnlyAfterPlacementGuards(t *testing.T) {
	...
	lower := candidate("lower_mid", "center", 300, 1045, 400, 110)
	upper := candidate("upper_mid", "center", 300, 645, 400, 110)
	safeTop := []design.Region{{X: 0, Y: 40, Width: 1080, Height: 200}}
	none := design.Region{}
	...
	if got := design.SelectAnchor([]design.Candidate{bottom, top}, none, nil, false, "", []design.Region{{X: 300, Y: 80, Width: 399, Height: 110}}); got != 0 {
	...
	if got := design.SelectAnchor([]design.Candidate{lower, top}, none, nil, true, "", []design.Region{lower.Plate}); got != 1 {
	...
	safeMid := []design.Region{upper.Plate, lower.Plate}
