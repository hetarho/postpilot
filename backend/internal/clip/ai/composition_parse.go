...
func parseCompositionPlan(cfg Config, input clip.PlanningInput, raw string) (out clip.EditPlan, err error) {
	...
	if err := attachCompositionCopy(cfg, input.Design.RegionPresets(), input.Ratio, doc, wire.Generated, timeline, portable, bindings, evidence, &plan, input.Instruction != ""); err != nil {
		return clip.EditPlan{}, err
	}
