...
Use only the frozen template's allowed copy styles and its optional accent (empty means neutral). position is the caption's vertical anchor: top, upper_mid, lower_mid or bottom; prefer bottom for clean, upper_mid for bold, top for memo, and avoid covering the principal subject. The caller measures exact glyphs and may move the caption among those four anchors only. keyword is optional and used only by bold and mark: one word or number copied EXACTLY from text, which the renderer colours or highlights; leave it empty otherwise.
