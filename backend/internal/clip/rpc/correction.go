...
func correctionPlan(p *v1.ClipEditPlan) clip.CorrectionPlan {
	...
	for _, c := range p.GetCuts() {
		copy := c.GetCopy()
		out.Cuts = append(out.Cuts, clip.CorrectionCut{ID: c.GetId(), SourceID: c.GetSourceId(), Fingerprint: c.GetFingerprint(), StartMS: int(c.GetStartMs()), EndMS: int(c.GetEndMs()), VolumePermille: int(c.GetVolumePermille()), Copy: clip.Caption{Text: copy.GetText(), Position: copy.GetPosition(), Style: copy.GetStyle(), Accent: copy.GetAccent(), StartMS: int(copy.GetStartMs()), EndMS: int(copy.GetEndMs())}})
	}
	return out
}
func editingProto(s *clip.CorrectionState) *v1.ClipEditingState {
	...
	for _, c := range s.Plan.Cuts {
		out.Plan.Cuts = append(out.Plan.Cuts, &v1.ClipEditCut{Id: c.ID, SourceId: c.SourceID, Fingerprint: c.Fingerprint, StartMs: int32(c.StartMS), EndMs: int32(c.EndMS), VolumePermille: int32(c.VolumePermille), Copy: &v1.ClipCaption{Text: c.Copy.Text, Position: c.Copy.Position, Style: c.Copy.Style, Accent: c.Copy.Accent, StartMs: int32(c.Copy.StartMS), EndMs: int32(c.Copy.EndMS)}})
	}
