...
func TestLayoutTakesTheRegionPresetFromTheProjectNotTheFrozenDocument(t *testing.T) {
	for _, chosen := range []string{"a", "b"} {
		...
		for _, visual := range layout.visuals {
			...
			if err := design.VerifyRegion("intro", chosen, "vertical", []string{"안녕하세요"}, visual.manifest.Parts); err != nil {
			...
			if design.VerifyRegion("intro", other, "vertical", []string{"안녕하세요"}, visual.manifest.Parts) == nil {
