...
func TestAutomaticRepairUsesOnlyRetainedGroundedAlternatives(t *testing.T) {
	plan := declaredPlan(t, `<clip version="1" intro="b" caption="bold" outro="e"><scene id="scene"><text id="copy" kind="ai" role="caption" basis="cut">Describe the scene.</text></scene><text id="empty-hook" kind="fixed" role="hook" basis="output-start"/><text id="empty-ending" kind="fixed" role="ending" basis="output-end"/></clip>`, "vertical")
...
func TestDeclaredBadgeInfoAndCardUseOnlyAuthoredText(t *testing.T) {
	...
	for _, ratio := range []string{"vertical", "horizontal", "square"} {
		t.Run(ratio, func(t *testing.T) {
			plan := declaredPlan(t, `<clip version="1" intro="b" caption="bold" outro="e"><text id="badge" kind="fixed" role="badge" position="header" basis="whole">  제작비 일부 지원  </text><text id="info" kind="fixed" role="info" position="bottom" basis="output-start" start="0" end="4"><row role="label">포함 사항</row><row role="caption">체험권</row></text><text id="card" kind="fixed" role="hook" basis="output-start" start="2" end="5"><row>직접 쓴 제목</row><row>오직 지정한 문구</row></text><text id="empty-ending" kind="fixed" role="ending" basis="output-end"/></clip>`, ratio)
...
func TestDeclaredCaptionUsesFixedTreatmentAndKeepsExplicitPosition(t *testing.T) {
	a, r := measured(t)
	plan := declaredPlan(t, `<clip version="1" intro="b" caption="bold" outro="e"><text id="one" kind="fixed" role="caption" position="top" align="left" basis="whole">첫 문장</text><text id="two" kind="fixed" role="caption" position="top" align="left" basis="whole">둘째 문장</text><text id="three" kind="fixed" role="caption" position="top" align="left" basis="whole">셋째 문장</text><text id="empty-hook" kind="fixed" role="hook" basis="output-start"/><text id="empty-ending" kind="fixed" role="ending" basis="output-end"/></clip>`, "vertical")
...
func TestInvalidLiteralReturnsItsIdentityWithoutTruncation(t *testing.T) {
	a, r := measured(t)
	plan := declaredPlan(t, `<clip version="1" intro="b" caption="bold" outro="e"><text id="owner" kind="fixed" role="info" basis="whole">절대로줄이거나생략하면안되는매우긴직접작성문구그대로모두남겨두고수정할수있어야합니다</text><text id="empty-hook" kind="fixed" role="hook" basis="output-start"/><text id="empty-ending" kind="fixed" role="ending" basis="output-end"/></clip>`, "vertical")
