		...
		t.Run(map[bool]string{false: "noisy-60s-speech", true: "variable-frame-rate-speech"}[vfr], func(t *testing.T) {
			...
			err := a.WithWorkspace(t.Context(), "profile", func(ws clip.MediaWorkspace) error {
				...
				if !vfr {
					...
					plan := clip.EditPlan{Ratio: "horizontal", DurationMS: 15000, Cuts: []clip.Cut{{ID: "profile-cut", SourceID: "profile", Fingerprint: "synthetic", EndMS: 15000, Focal: clip.Point{X: .5, Y: .5}, Copy: clip.Copy{Style: "clean", Anchor: "bottom", Align: "center"}}}}
