...
func New(cfg clip.MediaConfig, runner Runner) (*Adapter, error) {
	...
	if cfg.EncodeThreads < 1 || cfg.EncodeThreads > clip.MediaThreadMax {
		return nil, fmt.Errorf("clip encode threads must be between 1 and %d, got %d", clip.MediaThreadMax, cfg.EncodeThreads)
	}
	if cfg.DecodeThreads < 1 || cfg.DecodeThreads > clip.MediaThreadMax {
		return nil, fmt.Errorf("clip decode threads must be between 1 and %d, got %d", clip.MediaThreadMax, cfg.DecodeThreads)
	}
	if cfg.StaleAge <= 0 || cfg.OperationTimeout <= 0 || cfg.WaitDelay <= 0 || cfg.ChunkDurationMS <= 0 || cfg.ChunkDurationMS > 60000 || cfg.LongEdge <= 0 || cfg.LongEdge > 720 || cfg.FPS != 15 || cfg.StdoutLimit <= 0 || cfg.StderrLimit <= 0 || cfg.MaxStreams <= 0 || cfg.MaxDimension <= 0 || cfg.FFmpegPath == "" || cfg.FFprobePath == "" {
