...
// forced onto both.
func chargedApproval(kind string, payload []byte, user, id string) (GenerationApproval, bool) {
	switch kind {
	case "generate_clip":
		var p generationPayload
		if json.Unmarshal(payload, &p) != nil || !supportedGenerationPayload(p.Version) || p.ProjectID != id || p.Batch.UserID != user || p.Approval == nil || p.Approval.MaxCredits < 0 {
			return GenerationApproval{}, false
		}
