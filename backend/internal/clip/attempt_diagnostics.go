...
func SafeAttemptValues(values map[string]int) map[string]int {
	out := map[string]int{}
	for _, key := range []string{"source", "chunk", "cut", "cut_count", "target_ms", "before_ms", "after_ms", "remaining_ms", "min_ms", "max_ms", "transition_ms", "backward_ms"} {
...
func SafeAttemptPhase(phase string) string {
	switch phase {
	case "decode", "selection", "timeline_grow", "timeline_shrink", "timeline_total", "composition", "validation":
		return phase
	}
	return ""
}
